...
conf_path=../test/data/hermes.conf
...
hosts=${host1},${host2}
hermes_build_dir=${HOME}/dev/hermes/build
...
for h in ${host1} ${host2}; do
    docker exec --rm --user ${docker_user} -w ${hermes_build_dir} ${h} \
           bash -c "sed -e 's|\"\./\"|\""${docker_home}"\"|g' \
                        -e 's|\"localhost\"|\"node\"|' \
                        -e 's|rpc_num_threads = 1|rpc_num_threads = 4|' \
                        -e 's|{0, 0}|{1, 2}|' ${conf_path} > ${cluster_conf}"
    # Copy ssh keys to ${docker_home} .ssh dir
    # docker exec ${h} bash -c "cp ${HOME}/.ssh/id_rsa"
done
...
docker-compose exec -e GLOG_vmodule=rpc_thallium=1 --user ${docker_user} --privileged \
