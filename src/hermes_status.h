...
#define RETURN_CODES(X)                                                    \
  ...
  X(-5, INVALID_BLOB,                                                      \
    "Blob data is invalid. Please check "                                  \
    "address and size.")                                                   \
  ...
  X(-12, BUCKET_IN_USE,                                                    \
    "Bucket cannot be destroyed because its "                              \
    "reference count is greater than 1.")                                  \
  X(-13, DPE_RANDOM_FOUND_NO_TGT,                                          \
    "DPE random found no target with enough "                              \
    "space for blob.")                                                     \
  X(-14, DPE_GET_INVALID_TGT, "DPE got an invalid target ID.")             \
  X(-15, DPE_ORTOOLS_NO_SOLUTION,                                          \
    "DPE GLPK does not find a solution "                                   \
  ...
  X(-24, PLACE_SWAP_BLOB_TO_BUF_FAILED,                                    \
    "Place blob from swap space into "                                     \
    "buffering system failed.")                                            \
  X(-25, DPE_RR_FIND_TGT_FAILED,                                           \
    "DPE round-robin failed to find "                                      \
...
/** Hermes status codes. */
enum StatusCode { RETURN_CODES(RETURN_ENUM) };
...
class Status {
 public:
  ...
  bool Succeeded() const { return (status_ == HERMES_SUCCESS); }
  ...
  bool operator==(StatusCode code) { return (status_ == code); }

  /** Custom assignment operator from StatusCode */
  void operator=(StatusCode code) { status_ = code; }
