...
TEST_CASE("Open", "[process=" + std::to_string(info.comm_size) +
  ...
  SECTION("delete on close mode") {
    test::test_open(
        ...
        MPI_COMM_SELF);
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
    REQUIRE(stdfs::exists(info.new_file.c_str()));
    test::test_close();
    REQUIRE(!stdfs::exists(info.new_file.c_str()));
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
  }
...
TEST_CASE("OpenCollective", "[process=" + std::to_string(info.comm_size) +
  ...
  SECTION("delete on close mode on new file") {
    ...
    test::test_close();
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
    REQUIRE(!stdfs::exists(info.shared_new_file.c_str()));
  }
  TEST_INFO->Posttest();
}
...
TEST_CASE("SingleWrite", "[process=" + std::to_string(info.comm_size) +
  ...
  SECTION("write to existing file") {
    ...
    REQUIRE((size_t)TEST_INFO->size_written_orig_ == args.request_size);
    test::test_close();
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
  }
  ...
  SECTION("write to new  file") {
    ...
    test::test_close();
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
    REQUIRE(stdfs::file_size(info.new_file) == (size_t)TEST_INFO->size_written_orig_);
  }
  ...
  SECTION("write to new file with allocate") {
    ...
    REQUIRE(stdfs::file_size(info.shared_new_file) ==
            (size_t)TEST_INFO->size_written_orig_ * info.comm_size);
  }
  ...
  SECTION("delete on close mode on existing file") {
    auto original_size = stdfs::file_size(info.existing_file);
    test::test_open(info.existing_file.c_str(),
    ...
    test::test_write(info.write_data.c_str(), args.request_size, MPI_CHAR);
    REQUIRE((size_t)TEST_INFO->size_written_orig_ == args.request_size);
    REQUIRE(stdfs::exists(info.existing_file.c_str()));
    auto new_size =
        original_size > (size_t)TEST_INFO->size_written_orig_ * info.comm_size
            ? original_size
            : TEST_INFO->size_written_orig_ * info.comm_size;
    REQUIRE(stdfs::file_size(info.existing_file) == (size_t)new_size);
    test::test_close();
    REQUIRE(!stdfs::exists(info.existing_file.c_str()));
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
    check_bytes = false;
  ...
  SECTION("delete on close mode on existing file") {
    ...
    auto new_size =
        original_size > (size_t)TEST_INFO->size_written_orig_ * info.comm_size
            ? original_size
            : TEST_INFO->size_written_orig_ * info.comm_size;
    REQUIRE(stdfs::file_size(info.shared_existing_file) == (size_t)new_size);
    test::test_close();
    REQUIRE(!stdfs::exists(info.shared_existing_file.c_str()));
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
    check_bytes = false;
...
TEST_CASE("SingleRead", "[process=" + std::to_string(info.comm_size) +
  ...
  SECTION("read from existing file") {
    ...
    REQUIRE((size_t)TEST_INFO->size_read_orig_ == args.request_size);
    test::test_close();
    REQUIRE(TEST_INFO->status_orig_ == MPI_SUCCESS);
  }
