...
TEST_CASE("BatchedWriteRSRangeSmall",
  ...
  SECTION("write to new file always at the start") {
    ...
    REQUIRE(TEST_INFO->status_orig_ == 0);
    REQUIRE(stdfs::file_size(TEST_INFO->new_file_.hermes_.c_str()) == biggest_size_written);
  }
  ...
  SECTION("write to new file") {
    ...
    TEST_INFO->test_close();
    REQUIRE(TEST_INFO->status_orig_ == 0);
    REQUIRE(stdfs::file_size(TEST_INFO->new_file_.hermes_.c_str()) == total_size_written);
  }
  TEST_INFO->Posttest();
}
...
TEST_CASE("BatchedReadStrideFixedRSRangeSmall",
  ...
  SECTION("read from existing file") {
    ...
    for (size_t i = 0; i < TEST_INFO->num_iterations_; ++i) {
      size_t offset =
          (i * TEST_INFO->stride_size_) % (TEST_INFO->total_size_ - TEST_INFO->small_max_);
      TEST_INFO->test_seek(offset, SEEK_SET);
      REQUIRE(((size_t)TEST_INFO->status_orig_) == offset);
      size_t request_size =
          TEST_INFO->small_min_ + (rand_r(&TEST_INFO->rs_seed_) % TEST_INFO->small_max_);
...
TEST_CASE("BatchedReadRandomRSRangeLarge",
  ...
  SECTION("read from existing file") {
    ...
    for (size_t i = 0; i < TEST_INFO->num_iterations_; ++i) {
      ...
      size_t request_size =
          (TEST_INFO->large_min_ + (rand_r(&TEST_INFO->rs_seed_) % TEST_INFO->large_max_)) %
              (TEST_INFO->total_size_ - TEST_INFO->status_orig_);
      std::string data(request_size, '1');
      TEST_INFO->test_read(data.data(), request_size);
      REQUIRE(TEST_INFO->size_read_orig_ == request_size);
    }
...
TEST_CASE("BatchedReadStrideFixedRSRangeLarge",
  ...
  SECTION("read from existing file") {
    ...
    for (size_t i = 0; i < TEST_INFO->num_iterations_; ++i) {
      size_t offset =
          (i * TEST_INFO->stride_size_) % (TEST_INFO->total_size_ - TEST_INFO->large_max_);
      TEST_INFO->test_seek(offset, SEEK_SET);
      REQUIRE(((size_t)TEST_INFO->status_orig_) == offset);
      size_t request_size =
          TEST_INFO->large_min_ + (rand_r(&TEST_INFO->rs_seed_) % TEST_INFO->large_max_);
