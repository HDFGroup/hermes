#ifndef LABSTOR_op_min_max_LIB_EXEC_H_
#define LABSTOR_op_min_max_LIB_EXEC_H_
...
void Dup(u32 method, Task *orig_task, std::vector<LPointer<Task>> &dups) override {
  switch (method) {
    ...
    case Method::kCustom: {
      labstor::CALL_DUPLICATE(reinterpret_cast<CustomTask*>(orig_task), dups);
      break;
    }
  }
}
...
void DupEnd(u32 method, u32 replica, Task *orig_task, Task *dup_task) override {
  switch (method) {
    ...
    case Method::kCustom: {
      labstor::CALL_DUPLICATE_END(replica, reinterpret_cast<CustomTask*>(orig_task), reinterpret_cast<CustomTask*>(dup_task));
      break;
    }
  }
}
...
#endif  // LABSTOR_op_min_max_METHODS_H_