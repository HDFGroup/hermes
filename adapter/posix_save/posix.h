...
class API {
 public:
  typedef int(*opent)(const char * path, int flags,  ...);
  ...
  API() {
    ...
    if (is_intercepted) {
      open = (opent)dlsym(RTLD_NEXT, "open");
    } else {
      open = (opent)dlsym(RTLD_DEFAULT, "open");
    }
    if (open == nullptr) {
...
class PosixFS : public hermes::adapter::fs::Filesystem {
 private:
  File _RealOpen(AdapterStat &stat, const std::string &path);
  size_t _RealWrite(const std::string &filename, off_t offset,
