...
#ifndef HERMES_POSIX_ADAPTER_DATASTRUCTURES_H
#define HERMES_POSIX_ADAPTER_DATASTRUCTURES_H
...
/**
 * FileID structure used as an identifier in POSIX adapter.
 */
struct FileID {
  /**
   * attributes
...
/**
 * Structure POSIX adapter uses to define a file state.
 */
struct FileStruct {
  /**
   * attributes
...
/**
 * Structure POSIX adapter uses to define Hermes blob.
 */
struct HermesStruct {
  /**
   * attributes
...
/**
 * Stat which defines File within POSIX Adapter.
 */
struct AdapterStat {
  /**
   * attributes
...
/**
 * Define hash functions for POSIX Adapter.
 */
namespace std {
 ...
 */
template <>
struct hash<hermes::adapter::posix::FileID> {
  std::size_t operator()(const hermes::adapter::posix::FileID &key) const {
    std::size_t result = 0;
...
#endif  // HERMES_POSIX_ADAPTER_DATASTRUCTURES_H
