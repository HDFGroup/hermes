...
int simple_open(MPI_Comm &comm, const char *path, int &amode, MPI_Info &info,
  ...
  if (!existing.second) {
    LOG(INFO) << "File not opened before by adapter" << std::endl;
      ...
      hapi::Context ctx;
      stat.st_bkid =
	std::make_shared<hapi::Bucket>(path, mdm->GetHermes(), ctx);
      mdm->Create(fh, stat);
  } else {
...
int open_internal(MPI_Comm &comm, const char *path, int &amode,
...
int HERMES_DECL(MPI_File_read_at_all)(MPI_File fh, MPI_Offset offset,
                                       void *buf, int count,
                                       MPI_Datatype datatype,
                                       MPI_Status *status) {
  (void)fh;
...
int HERMES_DECL(MPI_File_write_ordered)(MPI_File fh, const void *buf,
                                         int count, MPI_Datatype datatype,
                                         MPI_Status *status)
{
  (void)fh;
...
int HERMES_DECL(MPI_File_iwrite_shared)(MPI_File fh, const void *buf,
                                         int count, MPI_Datatype datatype,
                                         MPI_Request *request)
{
  (void)fh;
