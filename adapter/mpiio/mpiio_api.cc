...
int HERMES_DECL(MPI_Init)(int *argc, char ***argv) {
  VLOG(kDebug) << "MPI Init intercepted." << std::endl;
  TRANSPARENT_HERMES
...
int HERMES_DECL(MPI_Waitall)(int count, MPI_Request *req, MPI_Status *status) {
  VLOG(kDebug) << "In MPI_Waitall." << std::endl;
  auto fs_api = HERMES_MPIIO_FS;
...
int HERMES_DECL(MPI_File_open)(MPI_Comm comm, const char *filename, int amode,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "Intercept MPI_File_open for filename: " << filename
            << " and mode: " << amode << std::endl;
  if (fs_api->IsPathTracked(filename)) {
    VLOG(kDebug) << "Intercept MPI_File_open: " << std::endl;
    AdapterStat stat;
    stat.comm_ = comm;
    stat.amode_ = amode;
    stat.info_ = info;
...
int HERMES_DECL(MPI_File_close)(MPI_File *fh) {
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_seek." << std::endl;
  if (fs_api->IsMpiFpTracked(fh)) {
    VLOG(kDebug) << "Intercept MPI_File_seek." << std::endl;
...
int HERMES_DECL(MPI_File_seek_shared)(MPI_File fh, MPI_Offset offset,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_seek_shared." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_seek_shared offset:" << offset
...
int HERMES_DECL(MPI_File_get_position)(MPI_File fh, MPI_Offset *offset) {
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_get_position." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_get_position." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_read_all)(MPI_File fh, void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_read_all." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_read_all." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_read_at_all)(MPI_File fh, MPI_Offset offset, void *buf,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_read_at_all." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_read_at_all." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_read_at)(MPI_File fh, MPI_Offset offset, void *buf,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_read_at." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_read_at." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_read)(MPI_File fh, void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_read." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_read." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_read_ordered)(MPI_File fh, void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_read_ordered." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_read_ordered." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_read_shared)(MPI_File fh, void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_read_shared." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_read_shared." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_write_all)(MPI_File fh, const void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_write_all." << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_write_all." << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_write_at_all)(MPI_File fh, MPI_Offset offset,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_write_at_all" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_write_at_all" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_write_at)(MPI_File fh, MPI_Offset offset,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_write_at" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_write_at" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_write)(MPI_File fh, const void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_write" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_write" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_write_ordered)(MPI_File fh, const void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_write_shared" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_write_ordered" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_write_shared)(MPI_File fh, const void *buf, int count,
  ...
  if (fs_api->IsMpiFpTracked(&fh)) {
    ...
    VLOG(kDebug) << "Intercept MPI_File_write_shared" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
    return fs_api->WriteOrdered(f, stat_exists, buf, count, datatype, status);
  }
...
int HERMES_DECL(MPI_File_iread_at)(MPI_File fh, MPI_Offset offset, void *buf,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_iread_at" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_iread_at" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_iread)(MPI_File fh, void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_iread" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_iread" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_iread_shared)(MPI_File fh, void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_iread_shared" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_iread_shared" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
    fs_api->ARead(f, stat_exists, buf, count, datatype, request);
    return MPI_SUCCESS;
  }
...
int HERMES_DECL(MPI_File_iwrite_at)(MPI_File fh, MPI_Offset offset,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_iwrite_at" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_iwrite_at" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_iwrite)(MPI_File fh, const void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_iwrite" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_iwrite" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_iwrite_shared)(MPI_File fh, const void *buf, int count,
  ...
  auto real_api = HERMES_MPIIO_API;
  auto fs_api = HERMES_MPIIO_FS;
  VLOG(kDebug) << "In MPI_File_iwrite_shared" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_iwrite_shared" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
...
int HERMES_DECL(MPI_File_sync)(MPI_File fh) {
  ...
  VLOG(kDebug) << "In MPI_File_iwrite_shared" << std::endl;
  if (fs_api->IsMpiFpTracked(&fh)) {
    VLOG(kDebug) << "Intercept MPI_File_sync" << std::endl;
    File f; f.hermes_mpi_fh_ = fh;
