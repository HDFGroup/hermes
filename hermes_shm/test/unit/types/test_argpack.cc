...
void test_argpack3() {
  ...
  PAGE_DIVIDE("") {
    hshm::PassArgPack::Call(
        hshm::make_argpack(T1(0), T2(1), T3(0)),
  ...
  PAGE_DIVIDE("") {
    T2 y = 1;
    hshm::PassArgPack::Call(
        hshm::make_argpack(0, y, 0),
        test_argpack3_pass<T1, T2, T3>);
  }

  // Create a 3-tuple
  PAGE_DIVIDE("") {
    hshm::tuple<T1, T2, T3> x(0, 1, 0);
    verify_tuple3(x);
  }

  // Copy a tuple
  PAGE_DIVIDE("") {
    hshm::tuple<T1, T2, T3> y(0, 1, 0);
  ...
  PAGE_DIVIDE("") {
    hshm::tuple<T1, T2, T3> x(0, 1, 0);
    hshm::ForwardIterateTuple::Apply(
        x,
  ...
  PAGE_DIVIDE("") {
    size_t y = hshm::MergeArgPacks::Merge(
                   hshm::make_argpack(T1(0)),
                   hshm::make_argpack(T2(1), T2(0))).Size();
    REQUIRE(y == 3);
  }
  ...
  PAGE_DIVIDE("") {
    hshm::PassArgPack::Call(
        hshm::MergeArgPacks::Merge(
            hshm::make_argpack(0),
            hshm::make_argpack(1, 0)),
        test_argpack3_pass<T1, T2, T3>);
  }

  // Construct tuple from argpack
  PAGE_DIVIDE("") {
    hshm::tuple<int, int, int> x(
        hshm::make_argpack(10, 11, 12));
    REQUIRE(x.Get<0>() == 10);
    REQUIRE(x.Get<1>() == 11);
    REQUIRE(x.Get<2>() == 12);
  }
  ...
  PAGE_DIVIDE("") {
    auto&& pack = hshm::ProductArgPacks::Product(
        0,
        hshm::make_argpack(1, 2),
        hshm::make_argpack<double, double>(3, 4));
    REQUIRE(pack.Size() == 4);
  }
  ...
  PAGE_DIVIDE("") {
    hshm::PassArgPack::Call(
        hshm::ProductArgPacks::Product(
            0,
            hshm::make_argpack(1, 2),
            hshm::make_argpack(3.0, 4.0)),
        test_product<
            hshm::ArgPack<int&&, int&&>,
